...
class Solution 
{
	...
	public List<Integer> rightSideView(BinaryTreeNode root) 
	{
        ...
        helper(root,0); // (root,level)
        return ans;
        
    }
    
    //Your supporting methods(if any) goes here
    private void helper(BinaryTreeNode root,int level){
        
        if(level == ans.size()){
            ans.add(root.data);
        }
        if(root.right != null && root.right.data != -1) helper(root.right,level+1);
        if(root.left != null && root.left.data != -1) helper(root.left,level+1);
        
    }
}

public class P2RightSideView 
{
