...
public class Day9P2FixedPoint {
    private static int binarySearch(int[] arr,int low,int high,int res){
        if(low > high){
            return res;  // target not found in the array
        }
        int mid = (low + high) / 2;

        if(arr[mid] == mid){
            // u found the target but there may be still smaller index which satisifies the condition 
            // go left and search for it
            res = mid;
            high = mid - 1;
        }

        if(arr[mid] < mid) return binarySearch(arr, mid + 1, high,res);

        else return binarySearch(arr, low, mid - 1,res);
    ...
    public static void main(String[] args){
        ...
        int result = binarySearch(arr, 0 , n - 1,-1);
